  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-2xl border border-subtle bg-surface shadow-sm overflow-hidden">
        <div className="flex items-center justify-between gap-3 px-4 py-2.5 border-b border-subtle bg-surface-muted/40">
          <p className="text-[11px] font-bold uppercase tracking-wider text-content-muted">
            {slots.length} voucher{slots.length === 1 ? '' : 's'}
          </p>
          <p className="text-[11px] text-content-muted">
            {slots.filter((s) => s.voucher).length} ready · {slots.filter((s) => !s.voucher).length} pending
          </p>
        </div>

        <div className="divide-y-0">
          {slots.map(({ type, meta, voucher, assignmentIndex, hotel }, i) => (
            <VoucherCompactCard
        ...
        {slots.length === 0 && (
          <p className="px-4 py-10 text-center text-sm text-content-muted">
            No voucher slots yet for this booking.
          </p>
        )}
      </div>
