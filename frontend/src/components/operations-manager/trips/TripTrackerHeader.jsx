...
import { formatDateRangeLabel } from './tripTrackerUtils';

export default function TripTrackerHeader() {
  return (
    <motion.div
    ...
    >
      <nav className="text-xs text-content-muted mb-2 flex items-center gap-1.5">
        ...
        <span>/</span>
        <span className="text-content-secondary font-medium">Trip Tracker</span>
      </nav>

      <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
        <div>
          <div className="flex items-center gap-2.5 mb-1">
            <MapPin className="w-7 h-7 text-blue-500 shrink-0" strokeWidth={2.25} />
            <h1 className="text-2xl sm:text-[26px] font-bold text-content-primary tracking-tight">
              Trip Tracker
            </h1>
          </div>
          <p className="text-sm text-content-secondary">
            Upcoming, ongoing, and completed trips with countdown
          </p>
        </div>
