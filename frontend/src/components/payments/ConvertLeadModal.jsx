...
export default function ConvertLeadModal({ open, onClose, leadId, onSuccess }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setReviewData({
        ...
        customerPhone: preview?.customerPhone || preview?.phone || preview?.whatsapp,
      });
    } finally {
      setSubmitting(false);
    }
  ...
  const handleReviewDone = ({ voucherSent } = {}) => {
    const payload = reviewData?.result;
    setReviewData(null);
    onSuccess?.({ ...payload, voucherSent });
    onClose?.();
  };

  const showPaymentForm = open && !reviewData;

  return (
    <>
      <AppModal open={showPaymentForm} onClose={onClose} size="3xl" lockDismiss={submitting}>
      ...
      <PaymentReceiptReviewModal
        ...
        bookingId={reviewData?.result?.booking?._id}
        paymentId={reviewData?.result?.payment?._id}
        summary={reviewData?.result?.summary}
        customerName={reviewData?.customerName}
        customerPhone={reviewData?.customerPhone}
        receiptNumber={reviewData?.result?.payment?.receiptNumber || reviewData?.result?.receipt?.receiptNumber}
